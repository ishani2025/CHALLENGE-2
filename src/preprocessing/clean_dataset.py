...
def build_stack_summary(stack_lines):
    ...
    summary = exception_lines[:2] + caused_by_lines[:2] + tail_frames
    return "\n".join(summary)
...
def main():
    ...
    cleaned_data = []
    for entry in data:
        entry["error_type"] = normalize_error_type(entry.get("error_type"))
        entry["stack_trace"] = clean_stack_trace(entry.get("stack_trace", []))
